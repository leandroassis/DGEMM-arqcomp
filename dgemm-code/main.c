...
int main(void){
    ...
    for(int i = 0; i < NUM_ITERACOES; i++){
        ...
        for(size_t size = TAMANHO_MIN_MATRIZES; size <= TAMANHO_MAX_MATRIZES; size+=STEP_MATRIZES){
            ...
            printf("Tempo de execução do algoritmo DGEMM normal com matriz de tamanho %zu: %.6f s.\n", size*size, t_exe[0]);
            ...
            printf("Tempo de execução do algoritmo DGEMM melhorado com matriz de tamanho %zu: %.6f s.\n", size*size, t_exe[1]);

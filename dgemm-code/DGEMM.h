...
#define STEP_MATRIZES        (TAMANHO_MAX_MATRIZES-TAMANHO_MIN_MATRIZES)/NUM_RESULTADOS_DESEJADOS

// valores inseridos na matriz
#define MIN_DOUBLE           -100.0
#define MAX_DOUBLE           100.0

#include <stdlib.h>

void dgemm(const size_t, const double*, const double*, double*);
void dgemm_optimized(const size_t, const double*, const double*, double*);

void aloca_matrizes(const size_t, double **, double **, double **);
void inicializa_matrizes(const size_t, double*, double*);
double rand_double();
